...
- プロジェクト初期化時に内蔵テンプレートを使用（テンプレートをバイナリに埋め込み、外部ファイルなしで初期化可能）
...
│   ├── commands/           # CLIコマンド実装
│   ├── config/             # 設定管理
│   ├── embedtemplate/      # 埋め込みテンプレート管理
│   │   └── template/       # バイナリに埋め込まれるテンプレート
...
- **commands**: CLI コマンドの実装（`sync`、`rename`、`watch`、`unwatch`、`init`）
- **embedtemplate**: バイナリに埋め込まれたテンプレートを管理
...
バイナリに埋め込まれたテンプレートにより、ユーザーは何も準備せずに`init`コマンドを実行できます。
開発中は自作のテンプレートを上記の方法（1～3）で指定することで使用できるため、テンプレートのカスタマイズも容易です。
...
プロジェクト初期化は、外部テンプレートが見つからない場合、バイナリに埋め込まれたテンプレートが自動的に使用されます。そのため、別途ファイルを用意しなくてもプロジェクトを初期化できます。

独自のテンプレートを使用したい場合は、テンプレート優先順位のセクションを参照してください。
...
**Q: プロジェクト初期化時にテンプレートが見つからない場合はどうなりますか？**
A: 埋め込みテンプレートが自動的に使用されます。バイナリに埋め込まれているため、常に利用可能です。